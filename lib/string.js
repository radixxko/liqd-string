...
module.exports = function( overload ) // if overload === undefined extend String
{
	...
	{
		return construct( this.replace( /[^A-Za-z0-9\[\] ]/g, c => LATIN[c] || c ));
  	});
	...
	{
		return this.underscore().replace(/_id$/,'').replace(/_/g, ' ').trim().capitalize();
    });
	...
	{
		return ( length >= 0 ? construct( this.substr( this.length - length )) : this.left( -length ));
    });

  	defineFunction( 'slugify', function()
  	{
  		return construct( this.latinise().replace(/[^\w\s-]/g, '').toLowerCase().dasherize().replace(/^\-+|\-+$/g, '' ));
	});
